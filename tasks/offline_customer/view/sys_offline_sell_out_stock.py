...
# @Time    : 2026/3/16
# @Author  : Zhu Yaming
# @File    : sys_offline_sell_out_stock.py.py
# @Description : 
"""
...
from apps.system.offline_customer.models import OfflineSellOutStock
...
from apps.system.reports.view.common_func import (get_translaiton_dict_from_request, report_translate_output,
                                                  apply_translation_map, download_data_optimized, get_df_cache_bysql)
from apps.system.offline_customer.models import OfflineCustomer, DataOfflineCustomerSku
from apps.system.offline_customer.view.sys_offline_sell_out_sales import get_customer_sku_dict
...
async def batch_update_offline_stock(
    request: Request,
    ...
    stmt = select(OfflineSellOutStock).where(OfflineSellOutStock._id.in_(ids))
    ...
    if update_data:
        ...
        if update_ids:
            await disable_sales_or_stock_by_ids(request, session, update_ids, "stock")
        ...
        async with inter_session.begin():
            for log_item in logs_to_record:
                ...
                await log_async_create(
                    ...
                    refer_type="offline_sell_out_stock",
                    refer_table="data_offline_sell_out_stock",
...
@report_translate_output(fields_to_translate=[])
async def upload_offline_sell_out_stock(
    request: Request,
    ...
    excel_cls_mapping = {
        "统计日期(Statistics Date)(YYYY-MM-DD/DD-MM-YYYY)": "sales_date",
        ...
        "在库库存数(In-Stock Quantity)": "stock_num"
    }

    cols = ["统计日期(Statistics Date)(YYYY-MM-DD/DD-MM-YYYY)", "客户编码(Customer Code)", "店铺名称(Store Name)", "店铺编码(Store Code)", "Proudct ID"]
    ...
    if "是否含税(Tax Included)" not in df.columns:
        df["是否含税(Tax Included)"] = 1
    else:
        df["是否含税(Tax Included)"] = df["是否含税(Tax Included)"].apply(lambda x: tax_dict.get(x, x))
    ...
    try:
        ...
        for row in records:
            ...
            if row['stock_num'] is None or row['stock_num'] == "":
                reason = "无效数据"
                if is_trans:
                    reason = translation_dict.get(reason, reason)
                error_list.append({
    ...
    if error_list:
        ...
        filename = f"Stock_Upload_Error_Records_{timestamp}.xlsx"
        result = await upload_df_to_oss(df_error, filename, file_path='operation/attachment/stock_error')
...
# API tag: 运营-客户管理-线下sellout-库存明细
...
async def get_offline_customer_stock_list(
    request: Request,
    nation: str = "[]",
    batch_number: str = "",
    customer_code: str = "[]",
    ...
    table_mapping = await async_get_one_title_mapping(inter_session, menu_id=305)
    ...
    if is_download:
        ...
        sio, headers = await download_data_optimized(header, data_col, data, f"Offline_Customer_Sell_Out_Stock_{int(time.time())}")
...
@report_translate_output(fields_to_translate=[])
async def batch_disabel_customer_stock(
    request: Request,
    nation: str = "[]",
    batch_number: str = "",
    customer_code: str = "[]",
    ...
    if all_ids:
        await disable_sales_or_stock_by_ids(request, session, all_ids, "stock")
