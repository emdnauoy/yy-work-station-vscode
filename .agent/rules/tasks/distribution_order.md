---
description: "分销下单子任务规则。编辑 tasks/distribution_order/ 下文件时自动附加。"
alwaysApply: false
globs: tasks/distribution_order/**
---
...
- **import 路径（尽量不用相对路径）**：
  - **禁止** `from ..`、`from ...` 及 `from .模块` 引用本任务内代码（含 `views` → 上级包）。
  - **统一**主仓绝对路径：`from apps.system.distribution_order.<模块> import ...`（与 `views/distribution_order.py` 一致）。
  - 主仓公共：`core.db.session`、`loguru` 等照旧；跨模块如线下客户：`from apps.system.offline_customer.models import ...`（**禁止** `from ..offline_customer`）。
